...
# ----------------------------------------------------------------------
# QR target — always routes through this app so validity can be checked
# ----------------------------------------------------------------------

def build_cert_qr_target(cert_id):
...
def main():
    ...
    with tabs[0]:
        ...
        base_url_set = bool(common.get_setting(DB_PATH, "base_url", "").strip())
        if not base_url_set:
            st.warning(
                "⚠️ App base URL is not set. Certificate QR codes require a permanent base "
                "URL so validity can be checked on scan — set it in the Settings tab before "
                "generating QR codes."
            )
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                ...
                data = build_cert_qr_target(row["cert_id"])
    ...
    with tabs[2]:
        ...
        if not base_url_set:
            st.warning(
                "⚠️ App base URL is not set (Settings tab) — certificates will still be "
                "extracted, renamed, and imported, but QR stamping and the master PDF will "
                "be skipped until it's set."
            )
                ...
                if st.button("Process selected certificates", type="primary"):
                    ...
                    for i, erow in edited.iterrows():
                        ...
                        if base_url_set:
                            ...
                            data = build_cert_qr_target(cert_id)
                        ...
                        link = None
                        if stamped_path and common.zoho_configured(DB_PATH):
    ...
    with tabs[3]:
        ...
        if df_all_certs.empty:
            ...
            if source is None:
                ...
                data = build_cert_qr_target(sel_cert_id)
                if not data:
                    st.warning("Set the App base URL in Settings before generating a QR to stamp.")
                else:
    ...
    with tabs[6]:
        ...
        st.markdown("""
        Certificate QR codes always encode a link back to *this app* (never the
        certificate file directly) — that's what lets validity be checked on every
        scan. This requires deploying the app at a permanent public URL.
        The verification endpoint is **publicly accessible** — no login is needed
        to follow a QR scan.
        """)
